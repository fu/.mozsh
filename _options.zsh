...
#setopt no_autoname_dirs     # after foo=/absolute/path, ~foo may expand to
                            ...
                            # `~foo' until `~foo' has been used - WTF?
...
setopt nobeep               # beeps drive me mad!
...
setopt noclobber            # don't overwrite files .. e.g
                            # cat foo.txt > ~/.zshrc - will still work when used as
                            # cat foo.txt >! ~/.zshrc
...
setopt glob                 # sure i want globbing - dunno what it is but yea
...
#[ h-j ]########################################################################
setopt no_hash_cmds            # do not always search through path, hash cmds
setopt no_hash_dirs            # hash directories holding commands too
setopt no_hash_list_all        # verify path hash on completion
...
setopt rec_exact            # recognize exact match even if ambiguous
#setopt restricted           #
setopt no_rm_star_silent    # ask user before rm foo/*
setopt no_rm_star_wait      # dont't idle 10 seconds

#[ s-z ]########################################################################
setopt share_history        # all shells know all commands ...
...
setopt no_transient_rprompt    # better for cut/paste not so good to keep track of prompt time
